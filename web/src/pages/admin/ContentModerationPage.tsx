...
type Court = { id: number; name: string; cityId: number };
...
export default function ContentModerationPage() {
  ...
  async function loadCourts() {
    try {
      ...
      setError(e?.response?.data?.message ?? 'Failed to load courts');
    }
  }
  ...
  async function createCourt() {
    ...
    try {
      await api.post('/api/admin/courts', {
        name: newCourtName,
        cityId: newCourtCityId,
      });
      setNewCourtName('');
    ...
    <Stack spacing={3}>
              ...
              <Stack direction='row' alignItems='center' spacing={1.5}>
                <Box
                  sx={{
                    ...
                    height: 40,
                    borderRadius: 2,
                    bgcolor: 'rgba(var(--color-accent-rgb),0.1)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                  }}
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ bgcolor: 'var(--color-surface)' }}>
                <TableCell sx={{ fontWeight: 600, color: 'var(--color-text)' }}>
                  ID
                </TableCell>
                <TableCell sx={{ fontWeight: 600, color: 'var(--color-text)' }}>
                  City Name
            ...
            <TableBody>
              {cities.length === 0 ? (
                <TableRow>
                  ...
                  >
                    No cities found. Data will appear when the backend is
                    running.
                  </TableCell>
                </TableRow>
              ) : (
      ...
      >
        ...
        >
          ...
          >
            All courts currently available in the system
          </Typography>
        </Box>
        <TableContainer>
          <Table>
            ...
            <TableBody>
              {courts.length === 0 ? (
                <TableRow>
                  ...
                  >
                    No courts found. Add cities first, then create courts.
                  </TableCell>
                </TableRow>
              ) : (
                courts.map((c) => (
                  <TableRow key={c.id} hover>
                    ...
                    <TableCell
                      sx={{ color: 'rgba(var(--color-text-rgb),0.7)' }}
                    >
                      {cities.find((ct) => ct.id === c.cityId)?.name ??
                        `City #${c.cityId}`}
                    </TableCell>
                  </TableRow>
                ))
